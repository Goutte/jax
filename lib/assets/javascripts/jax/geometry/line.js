      ...
      if (Math.abs(D) < Math.EPSILON) { // S1 and S2 are parallel
        ...
        if (du == 0) { // +this+ is a single point
          ...
          if (dest)
            ...
            else vec3.set(dest.a, this.a);
          // vec3.copy(dest.b, this.b);
          return Jax.Geometry.INTERSECT;
        }
        ...
        if (dest) {
          if (isVec3) {
            ...
            vec3.add(dest, line.b, vec3.scale(tmp, v, t1));
            vec3.scale(dest, dest, 0.5);
          } else {
            vec3.add(dest, line.a, vec3.scale(dest.a, v, t0));
            vec3.add(dest, line.b, vec3.scale(dest.b, v, t1));
          }
        }
